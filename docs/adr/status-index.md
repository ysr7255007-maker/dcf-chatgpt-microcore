...
- `2026-07-20-dcf-first-install-static-survival.md` — **implementation candidate incomplete; page-level diagnostic truth and BrowserClaw live acceptance pending**; candidate state must not appear disabled, static survival must prevent silent disappearance, and registration health must never be mistaken for current-page execution health

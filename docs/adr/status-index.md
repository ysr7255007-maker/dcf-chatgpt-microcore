...
- `2026-07-20-dcf-managed-web-surface-runtime.md` — **accepted direction; BrowserClaw-first bootstrap spike pending**; BrowserClaw is the first managed Chromium substrate because it natively connects OpenCode/Codex through a local MCP endpoint, while DCF Surface Registration binds the remote provider conversation and ephemeral browser target to a durable DCF task; the remote webpage does not directly become an MCP client

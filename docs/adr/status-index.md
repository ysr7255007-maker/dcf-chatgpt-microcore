...
- `2026-07-20-dcf-managed-web-surface-runtime.md` — **accepted direction; BrowserOS-first installation and pairing spike pending**; the exact `browseros-ai/BrowserOS` Chromium fork is the first managed substrate candidate, while DCF Surface Registration binds the remote provider conversation and ephemeral BrowserOS page to a durable DCF task; the remote webpage does not directly become an MCP client

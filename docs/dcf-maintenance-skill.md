...
模块的安装、启用、运行投影和显示是不同事实。`hidden: true` 不得被 UI 表现为模块不存在。功能页必须提示隐藏模块，维护页必须能查看完整显示状态并通过用户 `moduleDisplay` 覆盖恢复；显示调整不得改写包定义或卸载模块。
...
“一键体检并复制”是完整诊断入口。涉及迁移、模块缺失、宿主监听、投影或运行状态的现场问题，先读取完整 `DCF_HEALTH_REPORT`，不要继续依赖用户口述。报告必须覆盖两种存储后端、root/hash/projection、包/模块/Surface、可见与隐藏模块数量、迁移差异、Host Adapter 和失败回执，同时不得包含对话正文、弹药正文、包 payload、命令参数或认证信息。

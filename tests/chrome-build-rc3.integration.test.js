...
assert.strictEqual(config.version, '1.0.0-rc.3');
...
assert(localAgent);
assert.strictEqual(localAgent.host_api, '3');
assert.strictEqual(localAgent.required, false);
assert.strictEqual(localAgent.default_enabled, false);
assert(!index.defaults.includes(localAgent.id));
assert(!fs.existsSync(path.join(extension, 'official/code-units.json')));
assert(fs.existsSync(path.join(extension, 'src/host-opencode.js')));
...
assert(extensionJs.includes('local_agent.session.prompt'));
...
console.log(JSON.stringify({ ok: true, pure_base: true, independent_plugins: 9, default_plugins: 8, local_agent_host_api: 3, dark_mode: true, next_migration_only: true }, null, 2));

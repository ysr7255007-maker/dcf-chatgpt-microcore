...
global.chrome = {
  ...
  permissions: {
    ...
    async request() { granted = true; return true; }
  },
  runtime: { onMessage: listeners(), onUserScriptMessage: listeners() }
};
...
global.DCFHost = { C: { nowIso: () => '2026-07-17T00:00:00.000Z', HOST_VERSION: '1.0.0-rc.3' } };
...
(async () => {
  ...
  await assert.rejects(() => H.handleLocalAgentMessage({ type: 'local_agent.config.set', config: { base_url: 'http://localhost:4096' } }, sender), /origin_is_fixed/);
  ...
  console.log(JSON.stringify({ ok: true, fixed_origin: true, trusted_sender: true, optional_host_permission: true, session_only_secret: true, basic_auth: true, session_prompt: true, permission_reply: true }, null, 2));

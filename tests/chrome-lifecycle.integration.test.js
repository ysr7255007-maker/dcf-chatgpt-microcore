...
(async () => {
  ...
  assert.strictEqual(install.downloaded, 12);
  let state = await H.storageGet();
  assert(state.snapshots.candidate);
  assert.strictEqual(registrations.length, 12);
  assert.strictEqual(worlds.length, 12);
  ...
  console.log(JSON.stringify({ ok: true, github_default_install: 12, startup_evidence_commit: true, one_shell_plugin_update: true, unchanged_plugin_refs: 11, base_update_check: true, generic_plugin_storage: true }, null, 2));

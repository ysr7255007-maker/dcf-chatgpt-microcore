...
(async () => {
  assert.strictEqual(index.units.length, 9);
  assert.strictEqual(index.defaults.length, 8);
  const localRef = index.units.find((unit) => unit.id === 'dcf.firstparty.local-agent');
  assert(localRef);
  assert.strictEqual(localRef.default_enabled, false);

  const install = await H.checkRemoteUpdates('test-install');
  assert.strictEqual(install.ok, true);
  assert.strictEqual(install.downloaded, 9);
  let state = await H.storageGet();
  assert(state.snapshots.candidate);
  assert.strictEqual(state.snapshots.candidate.entries.length, 9);
  assert.strictEqual(state.snapshots.candidate.entries.find((entry) => entry.id === localRef.id).enabled, false);
  assert.strictEqual(registrations.length, 8);
  assert.strictEqual(worlds.length, 8);
  const candidateId = state.snapshots.candidate.id;
  for (const entry of state.snapshots.candidate.entries.filter((item) => item.enabled !== false)) {
    await H.recordUnitStarted({ unit_id: entry.id, version: entry.version }, { tab: { id: 1 }, url: 'https://chatgpt.com/c/test' });
  }
  ...
  assert.strictEqual(state.snapshots.current.entries.find((entry) => entry.id === localRef.id).enabled, false);
  ...
  Object.assign(updatedIndex.units.find((unit) => unit.id === shellRef.id), { version: '1.0.0-rc.2-shell.5-test', hash: nextHash, code_url: 'https://raw.githubusercontent.com/ysr7255007-maker/dcf-chatgpt-microcore/test/shell.js' });
  ...
  state = await H.storageGet();
  assert.strictEqual(state.snapshots.candidate.entries.find((entry) => entry.id === shellRef.id).version, '1.0.0-rc.2-shell.5-test');
  for (const entry of state.snapshots.candidate.entries) {
    if (entry.id !== shellRef.id) assert.deepStrictEqual({ version: entry.version, hash: entry.hash, enabled: entry.enabled }, { version: previousRefs[entry.id].version, hash: previousRefs[entry.id].hash, enabled: previousRefs[entry.id].enabled });
  }
  ...
  console.log(JSON.stringify({ ok: true, downloaded_plugins: 9, default_registrations: 8, local_agent_stored_disabled: true, startup_evidence_commit: true, one_shell_plugin_update: true, unchanged_plugin_refs: 8, base_update_check: true, generic_plugin_storage: true }, null, 2));

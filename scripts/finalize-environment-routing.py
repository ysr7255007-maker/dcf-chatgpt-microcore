...
# App persistent controls use the formal Environment Reconciler surface.
...
# Maintenance Profile and restore operations use the same reconciler.
replace_once('src/modules/maintenance.js',
...
# Command runner keeps one-time Host Effects separate but routes every persistent step through Environment Reconciler.
Path(ROOT / 'src/runtime/commands.js').write_text(r''' 'use strict';
...
# Boot wiring supplies Reconciler everywhere and exposes a live read-only Environment Facade.

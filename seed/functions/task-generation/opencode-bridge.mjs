...
/**
 * Tolerant JSON repair for common LLM slips (currently: trailing commas
 * before } or ]). Returns a best-effort cleaned string; the caller still
...
=== END DCF 标准化输出契约 ===`;
            ...
            const checkFile = () => {
                ...
                try {
                    ...
                    try {
                        ...
                        // Try a tolerant repair (trailing commas, common LLM slips).
                        try {
                            data = JSON.parse(repairJson(raw));
                        } catch (_) {
                            // Still invalid. If the file content has been stable for a
                            // while it is complete-but-malformed — fail fast instead of
                            // waiting out the whole timeout.
                            if (lastRaw === raw) {
                                if (!invalidSince) invalidSince = Date.now();
                                if (Date.now() - invalidSince > 8000) {
                                    resolved = true;
                                    cleanup();
                                    resolve({ ok: false, invalid: true, reason: 'result file is not valid JSON', output_path });
                                    return;
                                }
                            } else {
                                invalidSince = null;
                            ...
                            lastRaw = raw;
                            return; // might be partially written — keep waiting
                        }
                    }
     ...
     * @returns {Promise<Array<{id, sessionID, permission, patterns}>>}
     */
    async listPermissions() {
        try {
            const res = await this._api('GET', '/permission', null, 5000);
    ...
    async replyPermission(requestId, reply = 'always') {
        try {
            const res = await this._api('POST', `/permission/${encodeURIComponent(requestId)}/reply`, { reply }, 5000);
            return res.status < 400;
        } catch (_) { return false; }
    }

    /**
     * Auto-approve any permission request originating from the given session,
     * polling for up to durationMs. Makes unattended generation work even when
     * OpenCode's permission config has not been (re)loaded. Returns a stop fn.
     * @param {string} sessionId
     * @param {number} [durationMs=600000]
     */
    autoApproveSession(sessionId, durationMs = 600000) {
        const deadline = Date.now() + durationMs;
        const timer = setInterval(async () => {
            if (Date.now() > deadline) { clearInterval(timer); return; }
            const perms = await this.listPermissions();
            for (const p of perms) {
                if (p && p.sessionID === sessionId && p.id) {
                    await this.replyPermission(p.id, 'always');
                }
            }
        }, 1500);

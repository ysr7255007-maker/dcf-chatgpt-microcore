(function () {
  ...
  try {
    ...
    function registerPanel(panelHost) {
      ...
      const old = panels.get(id); old?.button.remove(); old?.host.remove();
      ...
      panelHost.hidden = true; panelHost.style.display = '';
      panels.set(id, { button, host: panelHost }); tabs.append(button); body.append(panelHost);
      if (!activeId || !panels.has(activeId)) activate(id);
    }
